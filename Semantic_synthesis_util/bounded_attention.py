...
import Semantic_synthesis_util.injection_utils as injection_utils
...
class BoundedAttention(injection_utils.AttentionBase):
    ...
    def __init__(
        ...
        self.use_cross_mask = use_cross_mask
    ...
    def forward(self, q, k, v, is_cross, place_in_unet, num_heads, **kwargs): # enter point
        ...
        if is_cross:
            if self.use_cross_mask:
                ...
                masks = torch.zeros([batch_size, num_heads, n, d]).to(dtype=dtype,device=device)
        else:
            # masks.shape = (b, h, 4096, 4096), defualt = zero
        ...
        sim = sim.reshape(batch_size, num_heads, n, d) + masks  # o or -inf
    ...
    def _compute_loss_term(self, foreground_values, background_values):
        # mean over layers
        # if len(foreground_values) == 0 or len(background_values) == 0:
        #     return torch.zeros([1,2]).to("cuda")
        ...
        reward_base = (1 - (mean_foreground / (mean_foreground + mean_background))) ** 2
        penalty_base = torch.log(1 + mean_background)

        if self.alpha_desc:
            alpha = self._get_alpha_cosine()
            self.time_step_cnt += 1
        else:
            alpha = 0

        Loss = reward_base + alpha * penalty_base
        # iou = mean_background
        # print("t: ", self.time_step_cnt,"alpha: ", alpha, "reward_base: " ,reward_base, "alpha * penalty_base: ", alpha * penalty_base)
        return Loss
