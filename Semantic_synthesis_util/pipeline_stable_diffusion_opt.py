...
class StableDiffusionPipeline(StableDiffusionPipeline):
    ...
    def cfgpp_step(
        ...
        beta_prod_t = 1 - alpha_prod_t
        pred_x0 = (x - beta_prod_t**0.5 * model_output) / alpha_prod_t**0.5
        pred_dir = (1 - alpha_prod_t_prev)**0.5 * noise_pred_uncond
        x_prev = alpha_prod_t_prev**0.5 * pred_x0 + pred_dir
        return x_prev, pred_x0
    ...
    def use_lora(self, path=None):
        if path is None:
            return 
        inject_trainable_lora(self.unet, loras=path)
    ...
    def __call__(
        self,
        prompt,
        opt_init_prompt = False,
        opt_cfgpp = False,
        cfgpp_guidance_scale = 0.6,
        batch_size=1,
        height=512,
        width=512,
        ...
        **kwds):
        ...
        if opt_init_prompt:
            from util.promptist_util import load_promptist_model,optimize_text_prompt
            (
                promptist_model, 
                promptist_tokenizer
            ) = load_promptist_model(device=DEVICE)
        ...
        if do_classifier_free_guidance and neg_prompt is None:
        ...
        for i, t in enumerate(tqdm(self.scheduler.timesteps, desc="DDIM Sampler")):
            ...
            if do_classifier_free_guidance:
                ...
                noise_pred = noise_pred_uncon + (guidance_scale if not opt_cfgpp else cfgpp_guidance_scale) * (noise_pred_con - noise_pred_uncon)
            ...
            if not opt_cfgpp:
                ...
                latents, pred_x0 = self.cfgpp_step(noise_pred, t, latents, noise_pred_uncon)
